...
class Paginator(object):
    def __init__(self, ctx, pages=[], timeout=30, **options):
        ...
        self.color = options.get("color", discord.Color(value=0x7BCDE8))
    ...
    async def add_reactions(self):
        if not self.ctx.message: raise Exception("'-'")
    ...
    async def show_page(self, index: int):
        ...
        if not self.message:
            if self.show_page_count:
                self.embed.title = f"{self.current_page + 1}/{len(self.pages)}"
            if ".png" in self.pages:    
             self.embed.description = self.pages[0]
             self.message = await self.ctx.send(embed=self.embed)
            else:
             self.embed.set_image(url=self.pages[0])
             self.message = await self.ctx.send(embed=self.embed)
        else:
